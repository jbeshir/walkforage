/**
 * fetchCityGeology.ts - Fetch precision-5 geology data for major cities
 *
 * Creates a high-resolution (~5km) geology overlay for top global cities,
 ...
 *
 * Usage: npx tsx scripts/gis/fetchCityGeology.ts
 *
 * Output: scripts/gis/output/cities_geology.json
 */
...
async function main() {
  ...
  const outputPath = path.join(outputDir, 'cities_geology.json');
  ...
  for (let cityIdx = 0; cityIdx < cities.length; cityIdx++) {
    ...
    if ((cityIdx + 1) % 10 === 0) {
      const checkpointPath = path.join(outputDir, 'cities_geology_checkpoint.json');
      fs.writeFileSync(
        checkpointPath,
        JSON.stringify(
          {
